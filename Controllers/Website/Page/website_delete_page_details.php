	...
	function findWebsiteSubpages($page_id) {
		foreach(website::getAllWebsitePageSubpages($page_id) as $subpage) {
			if(count(website::getAllWebsitePageSubpages($subpage['page_id'])) > 0) {
				...
				deleteWebsiteSubpages($subpage['page_id']);
			}
		}
	}

	function deleteWebsiteSubpages($page_id) {
		foreach(website::getSpecificWebsitePageDetails($page_id, $_GET['lang_id']) as $page_details) {
			...
			website::deleteSpecificWebsitePageDetails($page_id, $_GET['lang_id']);
		}
	}
